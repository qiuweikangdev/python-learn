...
| `LLMChain` / `SequentialChain` / `RetrievalQA` 等 Chains 类 | LCEL 管道写法：`chain = prompt \| model \| parser` |
...
| **Chains** | 组合组件 | LCEL：`prompt \| model \| parser` | 根据复杂度选择 |
| **Memory** | 管理记忆 | LangGraph checkpointer（InMemorySaver） | 根据对话长度选择 |
| **Indexes** | 文档检索 | VectorStoreRetriever | 根据数据量选择 |
| **Agents** | 动态决策 | `create_agent` | 需要工具调用时使用 |
...
# 2. 创建带记忆的聊天Agent
# checkpointer 自动按 thread_id 保存完整对话历史，
# 取代旧版 ConversationChain + ConversationBufferMemory
...
print(response["messages"][-1].content)  # 能记住之前的信息
```

**设计要点：**
- 使用 LangGraph checkpointer（InMemorySaver）保存完整对话历史，取代旧版 ConversationBufferMemory
- 一个 `thread_id` 对应一个会话，多轮记忆自动维护
- 生产环境换用 SqliteSaver / PostgresSaver 持久化
...
# 5. 用 LCEL 组合问答链
# （旧版 RetrievalQA 已从主包移除，LCEL 写法更直观可控）
llm = ChatOpenAI(model="gpt-5-mini")
qa_chain = (
...
# 6. 使用：直接传入问题字符串
answer = qa_chain.invoke("文档的主要内容是什么？")
print(answer)
```
...
- 向量库使用独立集成包 langchain-chroma
- 用 LCEL 组合检索和问答，取代旧版 RetrievalQA 链
...
def calculate(expression: str) -> str:
    """计算数学表达式"""
    # 注意：eval 有代码注入风险，生产环境请用 ast.literal_eval
    # 或 simpleeval 等受限求值方案，此处仅为本地演示
    try:
...
# create_agent 内置标准提示，工具描述直接来自 docstring
llm = ChatOpenAI(model="gpt-5-mini")
agent = create_agent(
    llm,
    tools,
    system_prompt="""你是一个有用的AI助手。你可以使用工具：
- search_web: 搜索互联网信息
- calculate: 计算数学表达式
- get_weather: 获取天气信息
...
response = agent.invoke({
    "messages": [{"role": "user", "content": "北京今天天气怎么样？顺便帮我算一下123+456"}]
})
print(response["messages"][-1].content)
```
...
**设计要点：**
- 使用@tool装饰器定义工具
- 使用`create_agent`创建Agent（取代旧版 create_openai_functions_agent + AgentExecutor）
- 工具描述要清晰明确（docstring 即工具描述）
...
# 3. 用 LCEL 实现 map-reduce 总结
# （旧版 load_summarize_chain 已从主包移除，LCEL 写法透明可定制）
llm = ChatOpenAI(model="gpt-5-mini", temperature=0)

# map：分别总结每个块
map_prompt = ChatPromptTemplate.from_template("用三句话总结以下内容：\n\n{text}")
...
# 4. 使用
summaries = [map_chain.invoke({"text": doc.page_content}) for doc in chunks]
summary = reduce_chain.invoke(summaries)
print(summary)
```

**设计要点：**
- map-reduce：先分别总结每个块，再合并总结
- LCEL 写法每一步都透明，便于替换提示与模型
...
**设计要点：**
- 使用Pydantic定义输出格式
- `PydanticOutputParser` 从 `langchain_core.output_parsers` 导入
- 更简洁的替代：`llm.with_structured_output(PersonInfo)` 让模型直接返回 Pydantic 对象
