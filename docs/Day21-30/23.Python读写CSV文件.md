...
> **说明**：打开文件时的`newline=''`参数可以防止在 Windows 系统上生成的 CSV 文件出现多余的空行，这也是`csv`模块官方文档的要求；而`encoding='utf-8'`参数则可以防止中文内容在 Windows 默认的 GBK 编码下出现乱码。
...
> **注意**：上面的代码对`csvreader`对象做`for`循环时，每次会取出一个列表对象，该列表对象包含了一行中所有的字段。这里我们读取的是刚才用`|`作为分隔符生成的文件，所以传入了`delimiter='|'`；如果你的`scores.csv`是使用默认逗号分隔符生成的，请去掉`delimiter='|'`参数（或改为`delimiter=','`）。
