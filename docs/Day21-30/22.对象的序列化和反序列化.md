...
Python 标准库中的`json`模块对绝大多数应用场景来说已经足够好用，如果确实遇到了很高的序列化性能要求（例如需要处理海量的 JSON 数据），可以考虑性能更出色的三方库`orjson`，它的序列化和反序列化速度都显著快于标准库`json`，也快于早期的性能方案`ujson`（该库已经不再被推荐使用），而且`orjson`序列化的结果是`bytes`类型的对象，该项目至今仍在积极的维护之中。所谓三方库，是指非公司内部开发和使用的，也不是来自于官方标准库的 Python 模块，这些模块通常由其他公司、组织或个人开发，所以被称为三方库。Python 语言的标准库已经提供了诸多模块来方便我们的开发，但是对于一个强大的语言来说，它的生态圈一定也是非常繁荣的。

之前安装 Python 解释器时，默认情况下已经勾选了安装 pip，大家可以在命令提示符或终端中通过`pip --version`来确定是否已经拥有了 pip。Pip 是 Python 的包管理工具，通过 pip 可以安装、卸载、更新 Python 的三方库或工具，例如要安装上面提到的`orjson`，可以使用下面的命令（如果大家使用`uv`来管理依赖，等价的命令是`uv add orjson`）。

```Bash
pip install orjson
```

在默认情况下，pip 会访问`https://pypi.org/simple/`来获得三方库相关的数据，但是国内访问这个网站的速度并不是十分理想，因此国内用户可以使用清华大学提供的镜像来替代这个默认的下载源，再执行安装三方库的操作，命令如下所示。
...
可以通过`pip list`命令来查看已经安装过的三方库。如果想更新某个三方库，可以使用`pip install -U`或`pip install --upgrade`；如果要删除某个三方库，可以使用`pip uninstall`命令。需要特别说明的是，早期版本 pip 提供的`pip search`命令（根据名字查找三方库）已经被 PyPI 永久禁用，在现在版本的 pip 中执行该命令会直接报错；如果想要查找某个三方库，可以到 [PyPI](https://pypi.org/) 的网站上通过包名进行搜索；如果只是想查看某个包在 PyPI 上有哪些可用的版本，可以使用`pip index versions <包名>`命令（目前还是一个实验性命令），例如：

```Bash
pip index versions orjson
```

> **提示**：`pip index`执行时会输出一条 WARNING 提示，说明该命令目前还处于实验阶段，这属于正常现象，命令本身仍然会列出该包当前所有可用的版本。
