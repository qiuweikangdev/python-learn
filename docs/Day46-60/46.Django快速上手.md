...
```python
# 导入Django的HTTP响应类
# django.http.HttpResponse：用于返回HTTP响应的类
from django.http import HttpResponse


# 定义视图函数，处理HTTP请求并返回HTTP响应
# request参数：Django的HttpRequest对象，包含请求信息
def show_index(request):
    ...
    返回值：
        HttpResponse: 包含HTML内容的HTTP响应对象
    
    功能：返回一个简单的HTML标题
    """
    # 返回HttpResponse对象，包含HTML内容
    # <h1>标签：HTML一级标题
...
```Python
# 导入Django的admin模块和URL路径函数
# django.contrib.admin：Django管理后台模块
# django.urls.path：用于定义URL路径的函数
# django.urls.include：用于包含其他URL配置的函数
from django.contrib import admin
from django.urls import path, include

# 从first应用导入show_index视图函数
# first.views：first应用下的views模块
# show_index：视图函数名，处理首页请求
from first.views import show_index

# URL配置列表：定义URL路径与视图函数的映射关系
urlpatterns = [
    # admin/路径：映射到Django管理后台
    # admin.site.urls：Django管理后台的URL配置
    path('admin/', admin.site.urls),
    
    # hello/路径：映射到show_index视图函数
    # 当用户访问/hello/时，Django会调用show_index函数处理请求
...
def show_index(request):
    ...
    返回值：
        HttpResponse: 包含动态生成HTML内容的HTTP响应对象
    
    功能：随机选择3种水果并生成HTML列表
    """
    # 定义水果列表
    fruits = [
    ...
    # 使用sample函数从水果列表中随机选择3个不重复的水果
    # sample(fruits, 3)：从fruits中随机选择3个元素
    ...
    # 使用for循环遍历选中的水果，生成HTML列表项
    for fruit in selected_fruits:
        # f-string格式化：Python3.6+的字符串格式化方式
        # {fruit}：将变量fruit的值插入到字符串中
...
# 导入Django的render函数
# django.shortcuts.render：用于渲染模板并返回HTTP响应的便捷函数
...
def show_index(request):
    ...
    返回值：
        HttpResponse: 渲染模板后的HTTP响应对象
    
    功能：使用模板引擎渲染HTML页面，展示随机选择的水果
    """
    # 定义水果列表
    fruits = [
        'Apple', 'Orange', 'Pitaya', 'Durian', 'Waxberry', 'Blueberry',
    ...
    # 使用render函数渲染模板
    # 参数1：request对象
    # 参数2：模板文件名（Django会在配置的模板目录中查找）
    # 参数3：上下文字典，传递给模板的数据
    # {'fruits': selected_fruits}：将选中的水果列表传递给模板
    # 模板中可以通过{{ fruits }}访问这个变量
...
```Python
# Django模板配置
# TEMPLATES配置：定义模板引擎和模板文件的查找路径
TEMPLATES = [
    {
        # BACKEND：指定使用的模板引擎
        # DjangoTemplates：Django内置的模板引擎
        'BACKEND': 'django.template.backends.django.DjangoTemplates',
        
        # DIRS：模板文件目录列表
        # os.path.join(BASE_DIR, 'templates')：拼接模板目录路径
        # BASE_DIR：Django项目根目录
        'DIRS': [os.path.join(BASE_DIR, 'templates'), ],
        
        # APP_DIRS：是否在应用目录中查找模板
        # True：Django会在每个应用的templates目录中查找模板
        'APP_DIRS': True,
        ...
        'OPTIONS': {
            ...
            # 上下文处理器会向模板上下文添加变量
            'context_processors': [
                # 调试上下文处理器，添加debug和sql_queries变量
                'django.template.context_processors.debug',
                # 请求上下文处理器，添加request变量
                'django.template.context_processors.request',
                # 认证上下文处理器，添加user和perms变量
                'django.contrib.auth.context_processors.auth',
                # 消息上下文处理器，添加messages变量
