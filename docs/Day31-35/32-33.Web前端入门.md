...
> **说明**：Vue 2 已经于 2023 年 12 月 31 日正式停止维护（EOL），2026 年的新项目请一律使用 Vue 3；Vue 2 时代的官方脚手架 vue-cli 也已被基于 Vite 的 create-vue 取代。下面的示例我们会同步给出 Vue 3 的写法，如果在网上查资料时看到 `new Vue({ el: '#app', ... })` 这种 Options API 的老写法，请注意甄别。
...
3. 条件与循环。
   ...
   <script>
   	const app = Vue.createApp({
   		...
   		data: {
   			products: [
   				{"id": 1, "name": "iPhone 16 Pro", "quantity": 20},
   				{"id": 2, "name": "华为 Mate 70", "quantity": 0},
   				{"id": 3, "name": "小米 15", "quantity": 50}
   			]
   		}
   }).mount('#app');
   </script>
...
##### 使用脚手架 - create-vue（基于 Vite）

Vue官方为商业项目开发提供了脚手架工具，早期的vue-cli已经停止维护，被官方基于 Vite 构建的 create-vue 取代，通过它可以省去手工配置开发环境、测试环境和运行环境的步骤，让开发者只需要关注要解决的问题。

1. 创建项目，命令行会交互式引导选择TypeScript、路由、状态管理等特性。
...
3. 工程化的Vue 3项目使用单文件组件（SFC，即`.vue`文件）来组织代码，官方推荐使用`<script setup>`配合组合式 API 的写法。把脚手架生成的根组件`src/App.vue`改写成一个最小的计数器示例：
   ...
   > 说明：`ref()`用于创建响应式数据，在`<script setup>`中读写它的值要通过`count.value`，在模板中则会自动解包直接写`count`即可；`@click="count++"`是`v-on:click`的缩写。相比Vue 2中`new Vue({ el: '#app', data: {...}, methods: {...} })`的Options API写法，上面的组合式 API写法是Vue 3项目更推荐的选择。


#### UI框架 - [Element Plus](<https://element-plus.org/zh-CN/>)

Element Plus是基于 Vue 3 的桌面端组件库（老牌组件库 Element UI 的 Vue 3 版本），用于构造用户界面，支持响应式布局；基于 Vue 2.0 的 Element UI 已经随 Vue 2 停止维护，Vue 3 项目请使用 Element Plus。
...
4. 使用组件。
   ...
   const tableData = ref([
     { date: '2026-05-02', name: '王一霸', address: '上海市普陀区金沙江路 1518 弄' },
     { date: '2026-05-04', name: '刘二狗', address: '上海市普陀区金沙江路 1517 弄' },
     { date: '2026-05-01', name: '杨三萌', address: '上海市普陀区金沙江路 1519 弄' },
     { date: '2026-05-03', name: '陈四吹', address: '上海市普陀区金沙江路 1516 弄' }
   ])
   </script>
