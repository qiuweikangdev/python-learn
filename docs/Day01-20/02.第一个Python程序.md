...
> **提示**：在使用上面的命令安装 IPython 之前，可以先通过`pip config set global.index-url https://pypi.tuna.tsinghua.edu.cn/simple`命令将下载源修改为国内的清华镜像，否则下载安装的过程可能会非常的缓慢。
...
用 pip 安装三方库时，最让人头疼的就是下载速度的问题。这里再给大家介绍一个近几年迅速走红的工具 uv，它由 Astral 公司使用 Rust 语言编写，集解释器管理、虚拟环境、包管理等多种能力于一身，安装依赖的速度比 pip 快一个数量级，在 2026 年已经成为 Python 社区事实上的标准工具。macOS 和 Linux 系统可以在终端中执行下面的命令来安装 uv。

```bash
curl -LsSf https://astral.sh/uv/install.sh | sh
```

Windows 系统可以在 PowerShell 中执行下面的命令来安装 uv，如果不方便使用上面的方式，也可以直接执行`pip install uv`来安装。

```powershell
powershell -ExecutionPolicy ByPass -c "irm https://astral.sh/uv/install.ps1 | iex"
```

安装完成后，先记住几个 uv 最常用的命令，如下所示。

```bash
uv python install 3.12    # 安装 Python 3.12 解释器
uv venv                   # 在当前目录创建虚拟环境（默认目录名为 .venv）
uv pip install requests   # 向虚拟环境中安装三方库
uv run example01.py       # 使用项目的虚拟环境运行 Python 程序
```

> **说明**：uv 的能力远不止上面这些，它可以替代 pip、virtualenv、pyenv 等工具的大部分用法，我们会在《番外篇：现代 Python 工程化入门》中为大家深入介绍。
...
创建项目的时候需要指定项目的路径并创建”虚拟环境“，我们建议每个 Python 项目都在自己专属的虚拟环境中运行。如果你的系统上还没 Python 环境，那么 PyCharm 会提供官网的下载链接，当你点击“Create”按钮创建项目时，它会联网下载 Python 解释器，如下图所示。如果不使用 PyCharm 这样的 IDE，也可以用 Python 标准库自带的 venv 模块手动创建虚拟环境：在项目目录下执行`python3 -m venv .venv`（Windows 系统为`python -m venv .venv`），即可创建一个名为`.venv`的虚拟环境目录；macOS 和 Linux 系统通过`source .venv/bin/activate`命令激活虚拟环境，Windows 系统通过`.venv\Scripts\activate`命令激活，不再使用时执行`deactivate`命令即可退出虚拟环境。推荐大家把虚拟环境目录命名为`.venv`，并将其加入`.gitignore`文件，避免将虚拟环境提交到代码仓库。
...
```python
# 使用print()函数输出字符串到控制台
# print()是Python内置函数，用于将指定的内容输出到标准输出设备（通常是屏幕）
# 参数：'hello, world' - 要输出的字符串内容，用单引号包围
print('hello, world')
```
...
```python
# 第一行：使用print()函数输出'hello, world'字符串
# print()函数可以输出任何Python支持的数据类型
print('hello, world')
# 第二行：使用print()函数输出'goodbye, world'字符串
# Python代码按行执行，每行一条语句，不需要分号结尾
print('goodbye, world')
```
...
# print('hello, world')  # 这行代码被注释掉了，不会执行
# 单行注释使用#开头，可以注释掉整行内容

# 使用print()函数输出中文字符串
# Python3默认支持Unicode，可以直接输出中文
print("你好，世界！")
```
