...
让爬虫程序隐匿自己的身份对编写爬虫程序来说是比较重要的，很多网站对爬虫都比较反感，因为爬虫会耗费掉它们很多的网络带宽并制造很多无效的流量。当目标网站开始通过 IP 地址识别并封禁高频访问时，就需要借助**IP 代理**（市面上的商业代理服务如快代理、亮数据等），让被爬取的网站无法获取爬虫程序来源的真实 IP 地址，也就无法简单地通过 IP 地址对爬虫程序进行封禁。商业代理服务通常提供两类接入方式：一类是通过 API 接口按需提取一批代理 IP，由程序自行管理和轮换（也就是所谓的“代理池”）；另一类是**隧道代理**，服务商提供一个固定的代理入口（域名 + 端口），每次请求自动从背后的 IP 池中选出一个地址完成转发，使用简单，也是目前更主流的接入方式。

::: warning
使用代理服务（尤其是境外代理）时，请务必遵守所在地的法律法规以及代理服务商的服务条款，不要将代理用于任何违法违规的用途。
:::

无论是哪家服务商的隧道代理，接入方式都是统一的通用写法：把用户名、密码、代理主机和端口组装成`http://用户名:密码@代理主机:端口`这样的 URL，通过`requests`库的`proxies`参数传给请求即可，下面的代码演示了具体的用法。
...
# 隧道代理配置（示例值需要替换为你在代理服务商处开通的接入信息）
# PROXY_HOST：代理服务器地址（域名或IP）
# PROXY_PORT：代理服务器端口
# PROXY_USER：认证用户名
# PROXY_PASS：认证密码
...
# 标准的代理接入写法：scheme://用户名:密码@主机:端口
# 市面上的隧道代理都是通过HTTP代理协议转发的，http和https统一填写http://开头的
# 代理地址即可，HTTPS请求会通过CONNECT方式经由代理建立隧道
proxies = {
...
for page in range(1, 11):
    ...
    resp = requests.get(
        ...
        proxies=proxies,
        # timeout参数：设置超时时间（秒），避免代理不可用时程序长时间等待
        timeout=10
    )
...
> **说明**：上面的代码需要把代理主机、端口、用户名和密码替换为你在代理服务商处开通的隧道代理接入信息。市面上的代理服务（如快代理、亮数据等）提供的隧道代理均采用上面这种通用的接入方式，具体的套餐和计费策略请以各服务商的官方说明为准；作为学习用途，可以先从按量计费的小额套餐开始体验。
