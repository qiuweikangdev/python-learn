...
::: warning 豆瓣风控提示
豆瓣对非浏览器请求的风控比较严格：不带 User-Agent 或请求频率过高时，很容易收到 418 或 403 状态码，持续抓取还可能导致 IP 被封禁。因此在下面的例子中，请务必在`settings.py`中配置`USER_AGENT`和`DOWNLOAD_DELAY`，并保持`ROBOTSTXT_OBEY = True`以遵守豆瓣的 robots 协议。本例仅用于学习 Scrapy 框架的用法，请控制抓取规模，不要给豆瓣的服务器造成压力。
:::
