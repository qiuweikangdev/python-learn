...
::: tip 深入篇说明
本板块是爬虫系列的**深入篇**，侧重原理与底层机制（并发编程、Selenium、Scrapy 框架内幕）。建议先完成主线课程《[爬虫系列（15 天实战）](/crawler/)》，再阅读本篇加深理解。
:::
...
1. 网络爬虫本身是一种正当的技术，搜索引擎、新闻聚合、舆情监控等产品都建立在爬虫技术之上，但它绝不是没有法律约束的“法外之地”。我国已经建立起较为完善的数据法律体系：《网络安全法》《数据安全法》（2021年9月起施行）、《个人信息保护法》（2021年11月起施行），以及《刑法》中关于非法获取计算机信息系统数据罪、侵犯公民个人信息罪等相关条款，共同为网络数据的采集和使用划定了明确的边界。
2. 抓取网页上公开可见的数据（而不是网站后台的私密敏感信息）只是合规的起点，并非全部。判断一次爬取行为是否合规，通常还要综合评估：数据中是否包含个人信息或商业秘密、采集手段是否绕过了网站的访问控制、抓取的内容是否受版权保护、数据将被用于何种用途等，这些问题在动手写爬虫之前都要先想清楚。
...
4. `builtwith`库（识别网站所用技术）和`python-whois`库（查询网站所有者）属于“了解即可”的辅助工具，安装方式分别是`pip install builtwith`和`pip install python-whois`，使用也非常简单（例如`builtwith.parse('https://www.python.org/')`、`whois.whois('python.org')`）。

   > **附注**：这两个库已经多年未维护，只能对目标站点做非常粗略的技术栈或域名信息探测，老教程中常用的示例站点 bootcss.com 也早已重定向，实际参考价值有限。在实际工作中，识别网站技术栈更多依赖 Wappalyzer 这类浏览器插件，或直接分析响应头中的`Server`、`X-Powered-By`等字段；查询域名注册信息则可以使用各域名注册商提供的 WHOIS 查询服务。
