...
# 导入pandas库
# pandas：Python的数据分析库，提供DataFrame和Series数据结构
# 安装：pip install pandas
import pandas as pd

# 创建Series对象
# pd.Series(data, index)：创建Series对象
# 参数：
#   data：数据，可以是列表、数组、字典等
#   index：索引，用于标识数据的标签
# 返回值：Series对象
...
```python
# 标量运算：对Series中的每个元素进行相同的运算
# 给ser1中的每个元素加10
# +=：加法赋值运算符，等价于 ser1 = ser1 + 10
ser1 += 10
...
```python
# 矢量运算：两个Series对象之间的运算
# 将ser1和ser2对应位置的元素相加
# Pandas会自动对齐索引，相同索引的值相加
# 如果某个索引只在一个Series中存在，结果为NaN（缺失值）
ser1 + ser2
```
...
```Python
# 使用整数索引按位置访问Series元素
# 当索引是自定义的标签时，推荐使用iloc属性按整数位置访问
# ser1.iloc[2]：访问第三个元素（位置为2）
# 返回值：该位置的数据值
ser1.iloc[2]
```
...
> **提示**：在 pandas 3.x 中，像`ser1[2]`这样用整数标量去访问自定义标签索引的`Series`会直接抛出`KeyError`异常，按位置访问请一律使用`iloc`，按标签访问则使用`loc`或方括号加标签，这也是官方推荐的无歧义写法。
...
```Python
# 使用自定义索引访问Series元素
# Series支持使用自定义的标签索引访问数据
# ser1['三季度']：访问索引为'三季度'的元素
# 返回值：该索引位置的数据值
ser1['三季度']
```
...
```Python
# 使用自定义索引修改Series元素
# ser1['一季度'] = 380：将索引为'一季度'的元素修改为380
# Series支持通过索引直接修改值
ser1['一季度'] = 380
...
```Python
ser4.ffill()  # 用空值前面的非空值填充（等价于旧版写法 fillna(method='ffill')）
```
...
需要提醒大家注意的是，`dropna()`和`fillna()`方法执行后默认都不会修改原来的`Series`对象，而是返回一个新的`Series`对象，因此在实际工作中，我们通常用“重新赋值”的方式把处理结果保存下来，代码如下所示。

```Python
ser4 = ser4.dropna()      # 将删除空值后的结果重新赋值给 ser4
ser4 = ser4.fillna(40)    # 将填充空值后的结果重新赋值给 ser4
```

> **说明**：pandas 早期的很多方法提供了一个`inplace`参数，用于在原对象上就地修改，但从 pandas 2.x 开始，该参数已呈弃用趋势（官方 PDEP-8 提案），尤其是在 pandas 3.0 中 Copy-on-Write 默认开启后，对切片或派生对象做 inplace 修改大多会失效或产生告警。官方推荐的写法是：单次`loc`定位、直接列赋值（`df['col'] = ...`）、`assign()`方法以及上面的“重新赋值”写法，后面我们接触到的相关代码都会遵循这一习惯。
