...
::: warning
需要说明的是，`echarts-countries-pypkg` 这一系列地图数据包已经多年没有更新和维护，上面的安装命令未必总能成功，而且包里的地图数据也比较陈旧。如果安装失败，或者需要更新的地图数据，可以考虑放弃 pyecharts 的 Python 封装，改用官方 ECharts 的 JavaScript 直接渲染地图，或者使用 plotly 等同样支持地图可视化的库来画图。
:::
