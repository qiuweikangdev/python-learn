...
> **版本基线**：本文基于 pandas 3.x（Copy-on-Write 默认开启），更新于 2026-09。
...
`pandas`模块的`read_sql`函数可以通过 SQL 语句从数据库中读取数据创建`DataFrame`对象，该函数的第二个参数代表了需要连接的数据库。从 pandas 2.x 开始，`read_sql`只对基于 [SQLAlchemy](https://www.sqlalchemy.org/) 的数据库连接提供完整支持，到 pandas 3.0 中，直接传入裸 DBAPI 连接（如`pymysql`的`Connection`对象）的旧写法已经被移除，所以我们推荐大家统一使用 SQLAlchemy 的方式接入数据库。首先需要安装`sqlalchemy`和作为 MySQL 驱动的`pymysql`两个三方库，如果尚未安装，可以在 Jupyter 单元格中执行魔法指令`%pip install sqlalchemy pymysql`，然后再运行下面的代码。
...
# 通过指定的URL（统一资源定位符）创建数据库引擎
# URL格式：mysql+pymysql://用户名:密码@主机:端口/数据库名
engine = create_engine('mysql+pymysql://root:123456@localhost:3306/school')
# 通过SQL从数据库二维表读取数据创建DataFrame
df5 = pd.read_sql('select * from tb_emp', engine, index_col='eno')
df5
```

::: warning
千万不要把数据库的真实凭据（用户名、密码、公网 IP 等）写进代码或文档，正确的做法是使用环境变量或配置文件来管理它们。上面的示例假设你已经用 Docker 在本地启动了 MySQL 8.x 数据库（MySQL 5.7 已停止维护，请使用 8.x），只需执行下面一条命令即可：

```Bash
docker run -d --name mysql -e MYSQL_ROOT_PASSWORD=123456 -p 3306:3306 mysql:8
```

示例中的`school`数据库一共有三张表，分别是：`tb_dept`（部门表）、`tb_emp`（员工表）、`tb_emp2`（员工表2），你可以自行建库建表并插入一些测试数据。
:::

> **说明**：在 pandas 2.0 之前的旧版本中，可以直接把`pymysql`创建的裸连接传给`read_sql`函数（`pd.read_sql(sql, conn)`），这种写法从 pandas 2.x 起被弃用、在 pandas 3.0 中被移除，这里仅作对照，新代码请不要这样写。
...
除了使用 SQL 语句，`read_sql`函数的第一个参数也可以直接是二维表的表名，相当于加载整张表的数据，代码如下所示。
...
在开始讲解`DataFrame`的属性和方法前，我们先从之前提到的`school`数据库中读取三张表的数据，创建出三个`DataFrame`对象，完整的代码如下所示。
