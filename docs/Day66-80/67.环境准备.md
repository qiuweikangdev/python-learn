...
2. 虚拟环境相关。
    ...
    - 指定 Python 版本创建虚拟环境：`conda create --name venv python=3.12`
    - 指定 Python 版本创建虚拟环境并安装指定依赖项：`conda create --name venv python=3.12 numpy pandas`
...
3. 包（三方库或工具）管理。
    ...
    > **说明**：在搜索、安装和更新软件包时，默认会连接到官方网站进行操作，如果觉得速度不给力，可以将默认的官方网站替换为国内的镜像网站，推荐使用清华大学的开源镜像网站。将默认源更换为国内镜像的命令是：`conda config --add channels https://mirrors.tuna.tsinghua.edu.cn/anaconda/pkgs/main/`和`conda config --add channels https://mirrors.tuna.tsinghua.edu.cn/anaconda/cloud/conda-forge/`。需要注意的是，早期的`pkgs/free`通道已经停止更新并被归档，请不要再添加和使用；如果需要换回默认源，可以使用命令`conda config --remove-key channels`。
...
除了 Anaconda 和 pip，近几年 Python 社区还出现了一个非常受欢迎的环境管理工具 [uv](https://docs.astral.sh/uv/)，它由 Rust 编写，目前已经成为了很多团队管理 Python 环境和依赖项的事实标准。uv 可以用一个单文件项目（`pyproject.toml` + `uv.lock`）同时管理 Python 解释器版本、虚拟环境和依赖项，安装依赖的速度也远远快于 conda 和 pip。安装 uv 的命令如下所示。

```Bash
curl -LsSf https://astral.sh/uv/install.sh | sh
```

安装完成后，就可以用 uv 创建虚拟环境并安装依赖，代码如下所示。
...
那么 conda 和 uv 该如何选择呢？简单的建议是：常规的数据分析、Web 开发等项目优先使用 uv，速度和体验都更好；只有当你需要依赖复杂的科学计算二进制包（如 MKL 加速的 NumPy、GPU 版本的库等）时，conda 才仍然是值得考虑的选择。
