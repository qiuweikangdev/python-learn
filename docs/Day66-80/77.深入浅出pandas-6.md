...
> **说明**：`freq=W`表示采样周期为一周，它会默认星期日是一周的开始；如果你希望星期一表示一周的开始，你可以将其修改为`freq=W-MON`；你也可以试着将该参数的值修改为`12h`，`ME`，`QE`等，看看会发生什么，相信你不难猜到它们的含义。需要注意的是，pandas 3.x 中表示月末、季末的频率别名是`ME`和`QE`（旧版的`M`、`Q`已被移除），小时等时间单位请使用小写的`h`（旧版的大写`H`同样已被移除）。
...
大家可能注意到了，每5天抽取1天有可能会抽中非交易日，那么对应的列都变成了空值，为了解决这个问题，可以先通过`asfreq`方法完成抽样，再调用`ffill`方法用相邻交易日的数据填充空值。

代码：

```Python
baidu_df.asfreq('5D').ffill()
```
...
```python
baidu_df.resample('ME').agg(['mean', 'std'])
```
